...
class Pose:
    """
    # Pose
    The same position can be represented in different ways.
    Different description methods are uniformly expressed as the `Pose` object
    ...
    def axis_angle(self) -> tuple[list[float], list[float], float]:
        ...
        ```
        Returns the axis-angle representation as a tuple of a list for translation,
        a list for the axis, and a float for the angle.
        """
...
class ArmState:
    ...
    Represents the state of a robot arm, including joint positions, velocities,
    accelerations, torques, end-effector poses, Cartesian velocities, and load state.
...
class Arm(Robot):
    ...
    Basic interface for robot arm behavior, including state query,
    load and coordinate system settings, and motion parameters.
...
class ArmRealtimeControl:
    ...
    def move_with_closure(
        self, closure: Callable[["ArmState", float], tuple["MotionType", bool]]
    ) -> None:
        """
    ...
    def control_with_closure(
        self, closure: Callable[["ArmState", float], tuple["ControlType", bool]]
    ) -> None:
        """
...
class ArmRealtimeControlExt:
    ...
    def move_cartesian_euler_with_closure(
        self,
        closure: Callable[["ArmState", float], tuple[list[float], list[float], bool]],
    ) -> None:
        """
